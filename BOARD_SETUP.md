...
| Modulino Pixels + Buzzer | สาย **Qwiic** เสียบช่อง Qwiic (ต่อพ่วงกันได้ เสียบทางเดียว) |
| ไมโครโฟน | USB เข้าบอร์ด (ผ่าน USB Hub ที่แจก) |

> ต้องครบทั้ง 3 จุด (ไฟ + Modulino + ไมค์) ระบบถึงทำงานครบ — ไม่ใช่แค่มีไฟอย่างเดียว

## ▶️ เริ่มการทำงาน + รู้ได้ไงว่ากำลังฟัง
...
- **สัญญาณ "กำลังฟัง":** ทุกครั้งก่อนอัดเสียง โค้ดจะสั่ง **ไฟน้ำเงิน + บี๊บสั้น** (ฟังก์ชัน `signal_listening`)
  ผู้ใช้จะได้รู้ว่าพูดตอนไหน → พูดจบรอแป๊บ → ไฟเปลี่ยนเป็น แดง/เหลือง/เขียว + บอก %
