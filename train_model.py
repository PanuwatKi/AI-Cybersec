...
# ห่อด้วย CalibratedClassifierCV เพื่อให้ค่า % "สมจริง" ขึ้น (ไม่เด้งไป 0/100 ตลอด)
# ผลคือโซนสีเหลือง (40-69%) มีโอกาสขึ้นจริงเมื่อข้อความก้ำกึ่ง
report_vec = make_vectorizer()
X_train_vec = report_vec.fit_transform(X_train)
X_test_vec = report_vec.transform(X_test)
report_clf = CalibratedClassifierCV(clone(best_clf), method="sigmoid", cv=5)
...
model = CalibratedClassifierCV(clone(best_clf), method="sigmoid", cv=5)
